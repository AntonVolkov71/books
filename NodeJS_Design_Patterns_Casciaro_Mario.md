...
- выходите при первой возможности - return, continue, break
- именованные обратные вызовы, промежуточные результаты передавать в аргументах
- разбивать код на модули, на небольшие функции (в том числе для переиспользования)
```
  if(errr) {
    return callback(err)
  }
...
- Начало -> Задание 1 -> Задание 2 -> Задание 3 -> Конец
- результат предыдущего задания может повлиять на выполнение следующего
- **два варианта решения**
  - последовательное выполнение набора заданий без цепочек и передачи результата - АД Обратных вызовов
  - передача результата одного задания на вход другого (Цепочка, конвейер, каскад)
  - последовательный обход элементов коллекции и выполнение асинхронного задания для каждого